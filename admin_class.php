...
class Action
{
	...
	function save_member()
	{
		...
		$data = '';
		foreach ($_POST as $k => $v) {
			if (!empty($v) && $k != 'start_date') {

				if (!in_array($k, array('id', 'plan_id', 'package_id'))) {
					if (empty($data))
						$data .= " $k='{$v}' ";
					else
						$data .= ", $k='{$v}' ";
		...
		if (empty($receipt_id)) {
			...
			while ($i == 1) {
				$rand = mt_rand(1, 9999);
				$rand = sprintf("%'08d", $rand);
				$chk = $this->db->query("SELECT * FROM members where receipt_id = '$rand' ")->num_rows;
				if ($chk <= 0) {
					$data .= ", receipt_id='$rand' ";
					$i = 0;
				}
			}
		}
